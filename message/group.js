...
module.exports = async(xinz, anj, welcome, left) => {
    ...
    if (anj.action === 'add'){
        if (anj.participants[0] === xinz.user.jid){
            ...
            xinz.sendMessage(anj.jid, `Terimakasih telah menginvite bot ini ke grup kamu\nNama bot: ${botName}\n\nUntuk menggunakan bot\nSilakan ketik #menu`, MessageType.text)
        } else if (isWelcome){
            try {
        ...
        xinz.sendMessage(anj.jid, await getBuffer(pic), MessageType.image, {caption: `Good Bye Beban Grup @${anj.participants[0].split("@")[0]}\nSemoga jiwa mu tenang di alam sana\n\nBye bye beban grup!!`, contextInfo: {"mentionedJid": [anj.participants[0]]}})
    }
}